...
- `expenses` — **가계부**(날짜 종속 안 됨, 전역 배열). 각 항목 `{id, label, amount(숫자), currency(JPY/KRW/USD/EUR), category(대분류), subcategory?(소분류), payer?, image?, note?, date?}`. **종목 = 대분류/소분류 2단계**: 대분류는 `식비·쇼핑·교통·관광·숙박·기타`(EXPENSE_MAJORS), 카페·이자카야·간식·편의점 등 식당류는 모두 **식비**의 소분류. `expenses` 탭(`#expense-root`)에서 `renderExpenses` 가 통화별 합계(필터 반영) + "+ 지출 추가"(상단) + **필터 바(대분류/소분류/날짜/정렬)** + 목록 렌더. 소분류 옵션은 선택된 대분류 내에서 추출. 정렬 추가순/날짜순/금액순. 뷰 상태 `expenseFilter{category,sub,date,sort}`(비영속). 구 단일 category 데이터는 `resolveCats`(=`splitCategory`+inferMajor)로 대분류·소분류 해석(점선 배지). 추가 모달: **날짜 기본=오늘**, 대분류 select + 소분류 입력 + 대분류별 소분류 빠른칩(SUB_SUGGESTIONS). (기존 16건은 저장 JSON 직접 마이그레이션 완료.) 영수증 사진(`image`) 첨부 가능(uploadImage 재사용, 클릭 시 라이트박스). Worker `addExpense`/`updateExpense`/`deleteExpense` + `cleanExpense` 로 정리. 일행 간 공유.
...
| `addExpense` | `label, amount, currency?, category?(대분류), subcategory?(소분류), payer?, image?, note?, date?` | `expenses` 에 지출 항목 추가 (cleanExpense 정리, currency 기본 JPY) |
...
- `openPlaceSearch(onPick, initialQuery)` / `ensureMapsLoaded()` / `setPlanItemAddr(li, addr)` — 추가·편집 모달의 "🔍 찾기" 가 구글맵 장소 검색 팝업을 띄움(Places `textSearch`, `window.TRIP_LOAD_MAPS` 로 Places 로드 보장). 결과를 클릭하면 팝업 내 **미니 지도(`.place-search-map`)에 마커로 위치 미리보기** + 항목 강조(`.is-selected`), 하단 **"✓ 이 장소 등록"** 클릭(또는 결과 더블클릭) 시 onPick → 이름·`coords`·`addr` 채움. addr 은 항목 이름 아래 서브라인. sapporo.js 가 `loadMapsApi` 를 `window.TRIP_LOAD_MAPS` 로 노출.
...
- `renderExpenses()` / `renderExpenseItem(e)` / `openExpenseModal(e)` — 가계부 탭(`#expense-root`) 렌더: 합계(필터 반영) + "+ 지출 추가"(상단) + 대분류/소분류/날짜/정렬 필터 바 + 지출 목록(배지 "대분류 · 소분류", 영수증 썸네일·✎ 편집). 모달: 내용·금액·통화·대분류(select)·소분류(입력+빠른칩)·결제자·날짜(기본 오늘)·메모·영수증 사진. add/update/deleteExpense 호출. init·syncAll 에서 호출. 보조: `resolveCats`/`splitCategory`/`inferMajor`/`todayStr`, 상수 `EXPENSE_MAJORS`/`SUB_SUGGESTIONS`, 뷰상태 `expenseFilter{category,sub,date,sort}`.
