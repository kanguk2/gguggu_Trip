...
- `expenses` — **가계부**(날짜에 종속 안 됨, 전역 배열). 각 항목 `{id, label, amount(숫자), currency(JPY/KRW/USD/EUR), category?, payer?, image?, note?, date?}`. `expenses` 탭(`#expense-root`)에서 `renderExpenses` 가 통화별 합계(필터 반영) + "+ 지출 추가"(상단) + **필터·정렬 바**(종목/날짜/정렬) + 목록 렌더. 종목·날짜 필터는 실제 데이터에서 추출, 정렬은 추가순/날짜순/금액순(뷰 상태 `expenseFilter`, 비영속). `category` 없는 기존 건은 `inferCategory(label)` 로 종목 추정해 분류·표시(점선 배지). 추가 모달은 **날짜 기본값=오늘(todayStr)**, 종목 빠른선택 칩(STD_CATEGORIES) 제공. 영수증 사진(`image`) 첨부 가능(uploadImage 재사용, 클릭 시 라이트박스). Worker `addExpense`/`updateExpense`/`deleteExpense` + `cleanExpense` 로 정리. 일행 간 공유.
...
- `renderExpenses()` / `renderExpenseItem(e)` / `openExpenseModal(e)` — 가계부 탭(`#expense-root`) 렌더: 합계(필터 반영) + "+ 지출 추가"(상단) + 종목/날짜/정렬 필터 바 + 지출 목록(영수증 썸네일·✎ 편집). 모달에서 내용·금액·통화·분류(빠른칩)·결제자·날짜(기본 오늘)·메모·영수증 사진 입력. add/update/deleteExpense 호출. init·syncAll 에서 호출. `inferCategory`/`expenseCategory`/`todayStr` 보조, `expenseFilter`(category/date/sort) 뷰 상태.
